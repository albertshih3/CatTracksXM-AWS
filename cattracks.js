...
function buildHome(queryStringParameters) {
    ...
    for (let i = 0; i < stopData.length; i++) {
        ...
        if (stopData[i].stop_id == 6 || stopData[i].stop_id == 4 || stopData[i].stop_id == 7 || stopData[i].stop_id == 8 || stopData[i].stop_id == 9 || stopData[i].stop_id == 10 || stopData[i].stop_id == 11 || stopData[i].stop_id == 26) {
    ...
    if (queryStringParameters != null && 'selectInitialStop' in queryStringParameters) {
        xmJson.elementFields = {
            "initiallyHidden": false,
            "items": getCards(queryStringParameters.selectInitialStop)
        };
    }
    else {
